...
function AITest(p) {
	...
	this.acceptTrade = function(tradeObj) {
		...
		tradeValue += 10 * tradeObj.getCommunityChestArkhamCard();
		tradeValue += 10 * tradeObj.getChanceArkhamCard();
		...
		if (tradeValue > 25) {
			...
			return new Trade(initiator, recipient, proposedMoney, property, tradeObj.getCommunityChestArkhamCard(), tradeObj.getChanceArkhamCard());
		}
	...
	// Determine whether to post bail/use get out of Arkham free card (if in possession).
	// Return: boolean: true to post bail/use card.
	this.postBail = function() {
		console.log("postBail");

		// p.Arkhamroll === 2 on third turn in Arkham.
		if ((p.communityChestArkhamCard || p.chanceArkhamCard) && p.Arkhamroll === 2) {
			return true;
		} else {
			return false;
		}
