function Game() {
	...
	var resetTrade = function(initiator, recipient, allowRecipientToBeChanged) {
		...
		if (initiator.communityChestArkhamCard) {
			currentTableRow = initiatorSideTable.appendChild(document.createElement("tr"));
			currentTableRow.onclick = tableRowOnClick;
			...
			currentTableCellCheckbox.title = "Check this box to include this Get Out of Arkham Free Card in the trade.";
			...
			currentTableCell.textContent = "Get Out of Arkham Free Card";
		} else if (recipient.communityChestArkhamCard) {
			currentTableRow = recipientSideTable.appendChild(document.createElement("tr"));
			currentTableRow.onclick = tableRowOnClick;
		...
		if (initiator.chanceArkhamCard) {
			...
			currentTableCell.textContent = "Get Out of Arkham Free Card";
		} else if (recipient.chanceArkhamCard) {
			currentTableRow = recipientSideTable.appendChild(document.createElement("tr"));
			currentTableRow.onclick = tableRowOnClick;
	...
	var readTrade = function() {
		...
		var money;
		var communityChestArkhamCard;
		var chanceArkhamCard;
		...
		var trade = new Trade(initiator, recipient, money, property, communityChestArkhamCard, chanceArkhamCard);
	...
	var writeTrade = function(tradeObj) {
		...
		if (document.getElementById("tradeleftcheckbox40")) {
			if (tradeObj.getCommunityChestArkhamCard() === 1) {
		...
		if (document.getElementById("tradeleftcheckbox41")) {
			if (tradeObj.getChanceArkhamCard() === 1) {
	...
	this.acceptTrade = function(tradeObj) {
		...
		isAPropertySelected |= tradeObj.getCommunityChestArkhamCard();
		isAPropertySelected |= tradeObj.getChanceArkhamCard();
		...
		if (tradeObj.getCommunityChestArkhamCard() === 1) {
			initiator.communityChestArkhamCard = false;
			recipient.communityChestArkhamCard = true;
			addAlert(recipient.name + ' received a "Get Out of Arkham Free" card from ' + initiator.name + ".");
		} else if (tradeObj.getCommunityChestArkhamCard() === -1) {
			initiator.communityChestArkhamCard = true;
			recipient.communityChestArkhamCard = false;
			addAlert(initiator.name + ' received a "Get Out of Arkham Free" card from ' + recipient.name + ".");
		}

		if (tradeObj.getChanceArkhamCard() === 1) {
			initiator.chanceArkhamCard = false;
			recipient.chanceArkhamCard = true;
			addAlert(recipient.name + ' received a "Get Out of Arkham Free" card from ' + initiator.name + ".");
		} else if (tradeObj.getChanceArkhamCard() === -1) {
			initiator.chanceArkhamCard = true;
			recipient.chanceArkhamCard = false;
			addAlert(initiator.name + ' received a "Get Out of Arkham Free" card from ' + recipient.name + ".");
		}
	...
	this.proposeTrade = function() {
		...
		var reversedTrade = new Trade(recipient, initiator, -money, reversedTradeProperty, -tradeObj.getCommunityChestArkhamCard(), -tradeObj.getChanceArkhamCard());
	...
	this.bankruptcy = function() {
		...
		if (p.chanceArkhamCard) {
			p.chanceArkhamCard = false;
			pcredit.chanceArkhamCard = true;
		}

		if (p.communityChestArkhamCard) {
			p.communityChestArkhamCard = false;
			pcredit.communityChestArkhamCard = true;
		}
...
function Player(name, color) {
	...
	this.Arkham = false;
	this.Arkhamroll = 0;
	this.communityChestArkhamCard = false;
	this.chanceArkhamCard = false;
	this.bidding = true;
	this.human = true;
	// this.AI = null;
...
// communityChestArkhamCard: integer, 1 means offered, -1 means requested, 0 means neither
// chanceArkhamCard: integer, 1 means offered, -1 means requested, 0 means neither
function Trade(initiator, recipient, money, property, communityChestArkhamCard, chanceArkhamCard) {
	// For each property and get out of Arkham free cards, 1 means offered, -1 means requested, 0 means neither.
	...
	this.getCommunityChestArkhamCard = function() {
		return communityChestArkhamCard;
	};

	this.getChanceArkhamCard = function() {
		return chanceArkhamCard;
	};
}
...
function updatePosition() {
	// Reset borders
	document.getElementById("Arkham").style.border = "1px solid black";
	document.getElementById("Arkhampositionholder").innerHTML = "";
	for (var i = 0; i < 40; i++) {
	...
	for (var x = 0; x < 40; x++) {
		...
		for (var y = turn; y <= pcount; y++) {
			...
			if (player[y].position == x && !player[y].Arkham) {
	...
	for (var i = turn; i <= pcount; i++) {
		if (player[i].Arkham) {
			document.getElementById("Arkhampositionholder").innerHTML += "<div class='cell-position' title='" + player[i].name + "' style='background-color: " + player[i].color + "; left: " + left + "px; top: " + top + "px;'></div>";
	...
	if (p.Arkham) {
		document.getElementById("Arkham").style.border = "1px solid " + p.color;
	} else {
...
function updateOwned() {
	...
	if (p.communityChestArkhamCard) {
		...
		HTML += "<tr class='property-cell-row'><td class='propertycellcheckbox'><input type='checkbox' id='propertycheckbox40' /></td><td class='propertycellcolor' style='background: white;'></td><td class='propertycellname'>Get Out of Arkham Free Card</td></tr>";
	...
	if (p.chanceArkhamCard) {
		...
		HTML += "<tr class='property-cell-row'><td class='propertycellcheckbox'><input type='checkbox' id='propertycheckbox41' /></td><td class='propertycellcolor' style='background: white;'></td><td class='propertycellname'>Get Out of Arkham Free Card</td></tr>";
	}
...
function chanceCommunityChest() {
	...
	if (p.position === 2 || p.position === 17 || p.position === 33) {
		...
		// Remove the get out of Arkham free card from the deck.
...
function gotoArkham() {
	var p = player[turn];
	addAlert(p.name + " was sent directly to Arkham.");
	document.getElementById("landed").innerHTML = "You are in Arkham.";
...
function payfifty() {
	...
	addAlert(p.name + " paid the $50 fine to get out of Arkham.");
	updateMoney();
	updatePosition();
}

function useArkhamCard() {
	var p = player[turn];
	...
	if (p.communityChestArkhamCard) {
		...
		// Insert the get out of Arkham free card back into the community chest deck.
	...
	} else if (p.chanceArkhamCard) {
		p.chanceArkhamCard = false;

		// Insert the get out of Arkham free card back into the chance deck.
	...
	addAlert(p.name + " used a \"Get Out of Arkham Free\" card.");
	updateOwned();
	updatePosition();
}
...
function showStats() {
	...
	for (var x = 1; x <= pcount; x++) {
		...
		if (p.communityChestArkhamCard) {
			...
			HTML += "<tr><td class='statscellcolor'></td><td class='statscellname'>Get Out of Arkham Free Card</td></tr>";
...
function land(increasedRent) {
	...
	// Go to Arkham. Go directly to Arkham. Do not pass GO. Do not collect $200.
	if (p.position === 30) {
		updateMoney();
		updatePosition();

		if (p.human) {
			popup("<div>Go to Arkham. Go directly to Arkham. Do not pass GO. Do not collect $200.</div>", gotoArkham);
		} else {
			gotoArkham();
		}
...
function roll() {
	...
	if (die1 == die2 && !p.Arkham) {
		...
		// If player rolls doubles three times in a row, send him to Arkham
			...
			if (p.human) {
				popup("You rolled doubles three times in a row. Go to Arkham.", gotoArkham);
			} else {
				gotoArkham();
			}
	...
	if (p.Arkham === true) {
		...
		if (die1 == die2) {
			...
			addAlert(p.name + " rolled doubles to get out of Arkham.");
...
function play() {
	...
	if (p.Arkham) {
		$("#landed").show();
		document.getElementById("landed").innerHTML = "You are in Arkham.<input type='button' title='Pay $50 fine to get out of Arkham immediately.' value='Pay $50 fine' onclick='payfifty();' />";

		if (p.communityChestArkhamCard || p.chanceArkhamCard) {
			document.getElementById("landed").innerHTML += "<input type='button' id='gojfbutton' title='Use &quot;Get Out of Arkham Free&quot; card.' onclick='useArkhamCard();' value='Use Card' />";
		}

		document.getElementById("nextbutton").title = "Roll the dice. If you throw doubles, you will get out of Arkham.";

		if (p.Arkhamroll === 0)
			addAlert("This is " + p.name + "'s first turn in Arkham.");
		else if (p.Arkhamroll === 1)
			addAlert("This is " + p.name + "'s second turn in Arkham.");
		else if (p.Arkhamroll === 2) {
			...
			addAlert("This is " + p.name + "'s third turn in Arkham.");
		}
...
window.onload = function() {
	...
	// Arkham corrections
	$("<div>", {id: "Arkhampositionholder" }).appendTo("#Arkham");
	$("<span>").text("Arkham").appendTo("#Arkham");

	document.getElementById("Arkham").enlargeId = "enlarge40";

	document.getElementById("enlarge-wrap").innerHTML += "<div id='enlarge40' class='enlarge'><div id='enlarge40color' class='enlarge-color'></div><br /><div id='enlarge40name' class='enlarge-name'>Arkham</div><br /><div id='enlarge40price' class='enlarge-price'><img src='images/jake_icon.png' height='80' width='80' alt='' style='position: relative; top: -20px;' /></div><br /><div id='enlarge40token' class='enlarge-token'></div></div>";

	document.getElementById("enlarge40name").innerHTML = "Arkham";

	// Create event handlers for hovering and draging.

	var drag, dragX, dragY, dragObj, dragTop, dragLeft;

	$(".cell-position-holder, #Arkham").on("mouseover", function(){
		$("#" + this.enlargeId).show();
